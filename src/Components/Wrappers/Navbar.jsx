    ...
    <div className="navbar bg-w text-c">
      <div className="navbar-start">
        <div className="dropdown">
          <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden bg-w">
          ...
          <ul tabIndex={0} className=" bg-w menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow rounded-box w-52">
            ...
            {isAuthenticatedUser && (
              <li>
                <Link to="/appointmentsUser" className="btn btn-xs mt-3 btn-info bg-hb text-w hover:text-c">
                  Mis Turnos
                </Link>
              </li>
            )}
            {isAuthenticatedUser && (
              <li>
                <Link to="/createappointmentsUser" className="btn btn-xs mt-3 btn-info bg-hb text-w hover:text-c">
                  Crear Turno
                </Link>
              </li>
            )}
            ...
            <li>
              <Link
                ...
                onClick={() => (isAuthenticatedAuditor || isAuthenticatedDoctor || isAuthenticatedUser ? LogOut() : LogIn())}>
                {isAuthenticatedAuditor || isAuthenticatedDoctor || isAuthenticatedUser ? "LogOut" : "LogIn"}
              </Link>
            </li>
          </ul>
      ...
      <div className="navbar-end hidden lg:flex">
        {isAuthenticatedAuditor && (
          <Link to="/auditorPage" className="btn mr-4 btn-info bg-hb text-w hover:text-c">
            Admin
          </Link>
        )}
        {isAuthenticatedDoctor && (
          <NavLink to="/appointmentsDoctor" className="btn mr-4 btn-info bg-hb text-w hover:text-c">
            Mis Turnos
          </NavLink>
        )}
        {isAuthenticatedUser && (
          <NavLink to="/appointmentsUser" className="btn mr-4 btn-info bg-hb text-w hover:text-c">
            Mis Turnos
          </NavLink>
        )}
        {isAuthenticatedUser && (
          <NavLink to="/createappointmentsUser" className="btn mr-4 btn-info bg-hb text-w hover:text-c">
            Crear Turno
          </NavLink>
        )}
        {!isAuthenticatedAuditor && !isAuthenticatedDoctor && !isAuthenticatedUser && (
